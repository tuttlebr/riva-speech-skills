...
from config import nlp_config, riva_config
...
channel = grpc.insecure_channel(RIVA_API_URL)
...
if enable_qa == "true":
    ...
    riva_test = (
        "I am Riva. I can talk about the weather. My favorite season is spring. I know the weather info "
        ...
        for i in range(len(resp.slots)):
            ...
            if slot_class and token:
                if slot_class == "weatherplace" or slot_class == "destinationplace":
                    entity = {"value": token, "confidence": score, "entity": "location"}
                else:
                    entity = {"value": token, "confidence": score, "entity": slot_class}
...
def get_riva_output(text):
    ...
    if "location" not in entities:
        if verbose:
            print(
                f"[Riva NLU] Did not find any location in the string: {text}\n"
        ...
        if verbose:
            print(
                f"[Riva NLU] NER response results: \n {resp_ner.results[0].results}\n"
            )
        ...
        for result in resp_ner.results[0].results:
            if result.label[0].class_name == "LOC":
                if verbose:
                    print(
                        f"[Riva NLU] Location found: {result.token}"
                    )  # Flow unhandled for multiple location input
                loc_count += 1
        ...
        if loc_count == 0:
            ...
            if resp.domain.class_name == "nomatch.none":
                ...
                if enable_qa == "true":
                    ...
                    riva_misty_profile = requests.get(
                        nlp_config["RIVA_MISTY_PROFILE"]
                    ).text  # Live pull from Cloud
...
def get_riva_output_qa_only(text):
    ...
    try:
        if enable_qa == "true":
            if verbose:
                print("[Riva NLU] Checking using QA API")
            riva_mark_KB = requests.get(
                nlp_config["RIVA_MARK_KB"]
            ).text  # Live pull from Cloud
