...
class initialState(State):
    ...
    def run(self, request_data):
        ...
        request_data.update({"response": self.construct_message(request_data, text)})
...
class userInput(State):
    ...
    def run(self, request_data):
        ...
        if response_intent:
            ...
            ) and response_intent != request_data["context"].get("intent", False):
                self.next_state = self.get_state(
                    intents_project.get(response_intent, False), None
                )
        ...
        if "intent" in request_data["context"]:
            ...
            self.next_state = self.get_state(
                intents_project.get(request_data["context"]["intent"], False), None
            )
            return
...
class checkWeatherLocation(State):
    def __init__(self, bot, uid):
        super(checkWeatherLocation, self).__init__("checkWeatherLocation", bot, uid)
    ...
    def run(self, request_data):
        ...
        if location:
            ...
            request_data.update(
                {"response": self.construct_message(request_data, text)}
            )
