...
class WeatherService:
    def __init__(self):
        self.access_key = os.getenv("WEATHERSTACK_ACCESS_KEY")
        self.days_of_week = {
            "monday": 0,
            "tuesday": 1,
            "wednesday": 2,
    ...
    def time_to_days(self, context):
        ...
        if "weathertime" in context["payload"]:
            ...
            if ctxtime == "week":
                if "weatherforecastdaily" in context["payload"]:
                    ctxtime = (
                        context["payload"]["weatherforecastdaily"].lower()
                        + " "
                        + ctxtime
                    )
                else:
        ...
        if ctxtime:
            ...
            if "now" in ctxtime:
                ...
                context["time"] = "in {} {}".format(
                    context["payload"]["weathertime"], ctxtime
                )
                if "week" in ctxtime:
                    return q * 7
    ...
    def query_weather(self, location, response):
        params = {"access_key": self.access_key, "query": location}
        try:
