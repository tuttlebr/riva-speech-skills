...
class Weather(State):
    ...
    def weather_forecast(self, ws, request_data, day):
        ...
        if response["success"]:
            return """{} it'll be {} in {}. The temperature will be {} degrees, the humidity {} percent """.format(
                request_data["context"]["time"].capitalize(),
                response["condition"],
                response["city"],
                response["temperature_c"],
                response["humidity"],
    ...
    def xxxfall(self, ws, xxx, request_data):
        ...
        if response["success"]:
            if self.is_xxxing(xxx, response["condition"]):
                ...
                return """In {} it is currently not {}ing.""".format(
                    response["city"], xxx
                )
        return DEFAULT_MESSAGE
    ...
    def xxxfall_forecast(self, ws, xxx, request_data, day):
        ...
        if response["success"]:
            if self.is_xxxing(xxx, response["condition"]):
                return """In {} {} it will {}.""".format(
                    response["city"], request_data["context"]["time"], xxx
                )
            else:
                return """In {} {} it is not expected to {}.""".format(
                    response["city"], request_data["context"]["time"], xxx
                )
        return DEFAULT_MESSAGE
    ...
    def xxxfall_historical(self, ws, xxx, request_data, day):
        ...
        if response["success"]:
            if self.is_xxxing(xxx, response["condition"]):
                return """In {} {} it was {}ing.""".format(
                    response["city"], request_data["context"]["time"], xxx
                )
            else:
                return """In {} {} it was not {}ing.""".format(
                    response["city"], request_data["context"]["time"], xxx
                )
        return DEFAULT_MESSAGE
    ...
    def xxx_forecast(self, ws, xxx, request_data, day):
        ...
        if response["success"]:
            if self.is_xxxing(xxx, response["condition"]):
                return """In {} {} it will be {}.""".format(
                    response["city"], request_data["context"]["time"], xxx
                )
            else:
                return """In {} {} it is not expected to be {}.""".format(
                    response["city"], request_data["context"]["time"], xxx
                )
        return DEFAULT_MESSAGE
    ...
    def xxx_historical(self, ws, xxx, request_data, day):
        ...
        if response["success"]:
            if self.is_xxxing(xxx, response["condition"]):
                return """In {} {} it was {}.""".format(
                    response["city"], request_data["context"]["time"], xxx
                )
            else:
                return """In {} {} it was not {}.""".format(
                    response["city"], request_data["context"]["time"], xxx
                )
        return DEFAULT_MESSAGE
    ...
    def precip(self, ws, xxx, request_data):
        ...
        if response["success"]:
            if self.is_xxxing(xxx, response["condition"]):
                return """In {} there are currently {} inches of {}.""".format(
                    response["city"], response["precip"], xxx
                )
            else:
                return """In {} it is currently not {}ing.""".format(
                    response["city"], xxx
    ...
    def precip_forecast(self, ws, xxx, request_data, day):
        ...
        if response["success"]:
            if self.is_xxxing(xxx, response["condition"]):
                return """In {} {} it will be {}ing the expected precipitation is {} inches.""".format(
                    response["city"],
                    request_data["context"]["time"],
                    xxx,
                    response["precip"],
                )
            else:
                return """In {} {} it is not expected to be {}ing.""".format(
                    response["city"], request_data["context"]["time"], xxx
                )
        return DEFAULT_MESSAGE
    ...
    def precip_historical(self, ws, xxx, request_data, day):
        ...
        if response["success"]:
            if self.is_xxxing(xxx, response["condition"]):
                return """In {} {} it was {}ing with {} inches.""".format(
                    response["city"],
                    request_data["context"]["time"],
                    xxx,
                    response["precip"],
    ...
    def run(self, request_data):
        ...
        if (
            request_data["context"]["intent"] == "weather.weather"
            or request_data["context"]["intent"] == "context.weather"
        ):
            if days > 0:
            ...
            request_data["context"]["intent"] == "weather.humidity"
            or request_data["context"]["intent"] == "weather.humidity_yes_no"
        ):
            if days > 0:
            ...
            message = request_data["context"][
                "answer_span"
            ]  # "This is a test QA response"
        ...
        request_data.update({"response": self.construct_message(request_data, message)})
